...
Current stance: future consideration. Do not introduce GORM or database-shaped domain models for the MVP.
...
Current stance: defer until the first scenario loader exists. Add a version field if scenarios become durable user-facing test assets.

## Admin API Scope

Which admin APIs are necessary for the MVP?

Current stance: implement only the admin endpoints needed by the current phase. Request logs, state dumps, scenario management, and clock control can wait until their consumers exist.
